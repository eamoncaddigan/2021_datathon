...
clean_dispositions_periods <- function(period, min_period, max_period) {
  ...
  clean_periods <- str_replace_all(clean_periods, "[[:blank:]]+", " ")
  clean_periods <- str_replace(clean_periods, "^ ", "")
  clean_periods <- str_replace_all(clean_periods, " $", "")
  ...
  clean_periods <- str_replace_all(clean_periods, "([[:digit:]]+) - 1/2", "\\1.5")
  clean_periods <- str_replace_all(clean_periods, "([[:digit:]]+) 1/2", "\\1.5")
  clean_periods <- str_replace_all(clean_periods, "([[:digit:]]+) and a half", "\\1.5") 
  
  # Tokenize "time in"/"time-in", "time served" and some typos
  clean_periods <- str_replace(clean_periods, "tiime", "time")
  clean_periods <- str_replace(clean_periods, "tim ", "time")
  clean_periods <- str_replace(clean_periods, "time[ -]?(served|in)", "time_served")
  clean_periods <- str_replace(clean_periods, "balance of backtime", "time_served")
  clean_periods <- str_replace(clean_periods, "back time", "time_served")
  
  # Standardize units
  clean_periods <- str_replace_all(clean_periods, "hrs", "hours")
  clean_periods <- str_replace_all(clean_periods, "hour(?!s)",  "hours")
  clean_periods <- str_replace_all(clean_periods, "day(?!s)",   "days")
  clean_periods <- str_replace_all(clean_periods, "month(?!s)", "months")
  clean_periods <- str_replace_all(clean_periods, "year(?!s)",  "years")
  ...
  clean_periods <- str_replace_all(clean_periods, ", ", " plus ")
  clean_periods <- str_replace_all(clean_periods, " and ", " plus ")
  clean_periods <- str_replace_all(clean_periods, " less ", " minus ")
  ...
  clean_periods_split <- str_split_fixed(clean_periods, "\\s?((\\bto\\b)|-)\\s?", 2)
  ...
  missing_units <-  str_extract(str_extract(clean_periods_split[is_missing_units, 2], 
                                            "^[[:digit:]]+(\\.5)? [[:alpha:]]+"), 
                                "[[:alpha:]]+$")
